 ...
 * @return vector<int> 
 */
int Table::getNextPageIndex(Cursor cursor)
{
    logger.log("Table::getNextPageIndex");
    if(cursor.binaryOperator == NO_BINOP_CLAUSE || !this->indexed || this->indexedColumnName != cursor.conditionColumnName)
        if( cursor.pageIndex == this->blockCount-1)
            return -1;
            return cursor.pageIndex+1;
}
...
Cursor Table::getCursor(string conditionColumnName, BinaryOperator binaryOperator, int conditionValue)
{
    Cursor cursor(this->tableName, -1, conditionColumnName, binaryOperator, conditionValue);
    int pageIndex = this->getNextPageIndex(cursor);
    cursor.getNext(pageIndex);
    return cursor;
}
